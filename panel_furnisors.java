...
public class panel_furnisors extends JPanel {
	...
	public panel_furnisors() {
		...
		tableModel.addColumn("Cod Client");
		tableModel.addColumn("Nume");
		tableModel.addColumn("Prenume");
		tableModel.addColumn("Localitate");
		...
		JLabel update_label_id=new JLabel("Cod:");
		...
		JLabel update_label_address=new JLabel("Localitate:");
	...
	public ArrayList<Client> client_list(){
		...
		try {
			...
			String display_query="select * from [GiFY8].[dbo].[Clienti]";
			Statement st=con.createStatement();
			ResultSet rs=st.executeQuery(display_query);
			
			Client client;
			while(rs.next()) {
				client=new Client(rs.getInt("ClientID"), rs.getString("Nume"), rs.getString("Prenume"), rs.getString("Adresa"), rs.getString("CodClient"));
	...
	public void display_clients_list(JTable table){
		
		ArrayList<Client> list=client_list();
		DefaultTableModel model=(DefaultTableModel)table.getModel();
		model.setRowCount(0);
		Object[] row=new Object[4];
		
		for(int i=0;i<list.size();i++){
			row[0]=list.get(i).getClient_code();
